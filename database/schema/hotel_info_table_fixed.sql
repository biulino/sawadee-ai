-- Hotel Information Configuration Table (PostgreSQL 10 Compatible)
...
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'tenants') THEN
        ALTER TABLE hotel_info ADD CONSTRAINT fk_hotel_info_tenant 
...
-- Trigger function for updated_at (PostgreSQL 10 compatible)
...
-- Create trigger (PostgreSQL 10 compatible)
CREATE TRIGGER hotel_info_updated_at
    BEFORE UPDATE ON hotel_info
    FOR EACH ROW
    EXECUTE PROCEDURE update_hotel_info_updated_at();
